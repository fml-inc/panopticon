#!/usr/bin/env bash
# E2E test for Codex CLI + Panopticon
set -euo pipefail
...
if [ -z "${OPENAI_API_KEY:-}" ]; then
  log_skip "OPENAI_API_KEY not set — skipping Codex E2E"
  exit 0
fi
...
panopticon install --target codex --proxy --force
...
codex exec "List the files in the current directory and read the contents of README.md. Summarize what you find." \
  --full-auto 2>&1 || log_info "Session 1 command exited (may be normal)"
sleep 5

log_info "Session 2: File write + bash"
codex exec "Create a file called hello.py that prints 'Hello from panopticon test', then run it with python3." \
  --full-auto 2>&1 || log_info "Session 2 command exited (may be normal)"
sleep 5
