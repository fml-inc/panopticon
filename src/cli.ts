...
async function install() {
  ...
  const target = getFlagValue("--target", "claude");
  ...
  if (desktop) {
    ...
    await installClaudeCode(pluginRoot, target);
  }
}
...
async function installClaudeCode(pluginRoot: string, target = "claude") {
...
/** Check if a port is in use. Returns the PID holding it (via lsof/ss) or null. */
...
async function doctor() {
  ...
  if (!isPortFree && !receiver.running) {
    console.log(
      "  WARNING: Port is in use, but Panopticon PID file indicates it is stopped.",
    );
    console.log(
