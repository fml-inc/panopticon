...
export function createSyncLoop(opts: SyncOptions): SyncHandle {
  ...
  async function syncMergedEvents(target: SyncTarget): Promise<boolean> {
    const wmKey = watermarkKey("hook_events", target.name);
    const wm = readWatermark(wmKey);

    const { rows, maxId } = readMergedEvents(wm, batchSize);
    ...
    for (const batch of batches) {
      if (batch.length > 0) {
        const payload = serializeMergedEvents(batch);
        await postOtlp(
          `${target.url}/v1/logs`,
          payload,
  ...
  async function syncUnmatchedLogs(target: SyncTarget): Promise<boolean> {
    ...
    for (const batch of batches) {
      if (batch.length > 0) {
        const payload = serializeUnmatchedLogs(batch);
        await postOtlp(
          `${target.url}/v1/logs`,
          payload,
          resolveHeaders(target),
    ...
    for (const target of opts.targets) {
      try {
        for (let i = 0; i < MAX_ITERATIONS_PER_TABLE; i++) {
          if (!(await syncMergedEvents(target))) break;
          hasMore = true;
        }
        for (let i = 0; i < MAX_ITERATIONS_PER_TABLE; i++) {
          if (!(await syncUnmatchedLogs(target))) break;
          hasMore = true;
        }
        for (let i = 0; i < MAX_ITERATIONS_PER_TABLE; i++) {
