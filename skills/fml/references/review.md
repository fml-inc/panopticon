...
After producing the review, if lingering questions remain about coding intent or choices, query FML local session data and, when available, synced FML sources to answer them. Do not query session history while producing the review. The review should reflect what the diff says, not what session history says. Only use session history for "why" questions that the code and commit messages did not resolve. Append any answers under a **Follow-up: FML-sourced intent** section after the Verdict.

## Querying FML Sessions

Always check the local session database first. Then, if the `fml` CLI is installed and authenticated in this environment, also check synced sessions with the same time window and keywords. Synced FML may be absent, logged out, or unable to reach the service; if so, note that synced sources were unavailable and continue with the local DB rather than failing the review.

For common local lookups, prefer purpose-built MCP tools:

- MCP `sessions`: list recent sessions with stats.
- MCP `timeline`: pull messages and tool calls for a session.
- MCP `search`: full-text search across events and messages.

Fall back to raw SQL via local MCP `query` for ad-hoc joins the subcommands do not cover. Key local tables:
...
FTS footgun: hyphens are not operators. `MATCH 'fml-review'` can fail with `no such column: review`; quote hyphenated terms as `MATCH '"fml-review"'`.
...
2. For keyword search across message content, join `messages_fts` to `messages` by rowid or use local MCP `search`.
3. Pull relevant conversations with MCP `timeline` or `SELECT role, content FROM messages WHERE session_id = ? ORDER BY ordinal`.
...
Prefer corroborated local plus synced evidence when both exist. If they disagree, state the discrepancy. If neither local nor synced FML covers the commit window, say so instead of guessing. Sanity-check local coverage with `SELECT datetime(MAX(started_at_ms)/1000,'unixepoch') FROM sessions` before declaring no coverage; `sessions.machine` tells you which host a session ran on.
...
After the verdict, if FML queries answered any intent questions, add a **Follow-up: FML-sourced intent** section summarizing what was found and which review items it confirmed, softened, or contradicted. If local and synced FML sources do not cover the branch's commit window, say so.
